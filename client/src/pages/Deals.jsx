...
import {
  ...
  FormControl, IconButton, Tooltip, Chip, Grid,
} from '@mui/material';
import { Add, Delete, Edit, PictureAsPdf, Search as SearchIcon } from '@mui/icons-material';
...
function DealCard({ deal, onEdit, onDelete }) {
  ...
  return (
    <Card ref={setNodeRef} style={style} {...attributes} {...listeners} sx={{
      mb: 1.5, cursor: 'grab', borderRadius: 3, transition: 'box-shadow 0.2s',
      ...
      <CardContent sx={{ py: 1.5, '&:last-child': { pb: 1.5 }, position: 'relative' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <Box sx={{ flex: 1 }}>
            <Typography variant="subtitle2" fontWeight={600}>{deal.title}</Typography>
            <Typography variant="body2" fontWeight={700} sx={{ color: '#7C3AED', mt: 0.3 }}>
              ${deal.value?.toLocaleString()}
            </Typography>
            {deal.company && <Typography variant="caption" color="text.secondary">{deal.company}</Typography>}
          </Box>
          <Box className="deal-actions" sx={{ opacity: 0, transition: 'opacity 0.2s', display: 'flex', gap: 0.5 }}>
            <IconButton size="small" onClick={(e) => { e.stopPropagation(); onEdit(deal); }}><Edit fontSize="small" sx={{ color: '#7C3AED' }} /></IconButton>
            <IconButton size="small" onClick={(e) => { e.stopPropagation(); onDelete(deal._id); }} color="error"><Delete fontSize="small" /></IconButton>
          </Box>
        </Box>
      </CardContent>
    </Card>
...
export default function Deals() {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Delete this deal?')) {
      try { await deleteDeal(id); showSnackbar('Deal deleted'); }
    ...
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3, flexWrap: 'wrap', gap: 2 }}>
        <TextField size="small" placeholder="Search deals..." value={search}
          onChange={(e) => setSearch(e.target.value)} sx={{ width: 250 }}
          InputProps={{ startAdornment: <SearchIcon sx={{ mr: 1, color: '#7C3AED' }} /> }} />
