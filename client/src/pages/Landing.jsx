...
const TECH = [
  { icon: <StorageIcon />, label: 'MongoDB Atlas', desc: 'Cloud database' },
  { icon: <DeviceHubIcon />, label: 'Express + Node', desc: 'REST API' },
  { icon: <CloudIcon />, label: 'Vercel', desc: 'Serverless deploy' },
];
  ...
  return (
    <Box sx={{ bgcolor: '#F7F5FF', minHeight: '100vh' }}>
      <AppBar position="fixed" sx={{ bgcolor: '#7C3AED', boxShadow: 'none', borderBottom: '1px solid rgba(255,255,255,0.12)' }}>
        <Toolbar>
          <Typography variant="h6" sx={{ fontWeight: 800, flexGrow: 1, letterSpacing: 1.5 }}>
      ...
      <Box sx={{ pt: 12, pb: 8, textAlign: 'center' }}>
        <Container maxWidth="md">
          <Typography variant="h2" fontWeight={800} sx={{ color: '#090F19', fontSize: { xs: '2rem', md: '3.5rem' }, lineHeight: 1.1, mb: 2 }}>
            Manage Your Pipeline.{' '}
            <Box component="span" sx={{ color: '#7C3AED' }}>Close More Deals.</Box>
          </Typography>
          <Typography variant="h6" color="text.secondary" sx={{ mb: 4, fontWeight: 400, maxWidth: 600, mx: 'auto', lineHeight: 1.6 }}>
            ...
            {STATS.map((s) => (
              <Grid item xs={6} md={3} key={s.label}>
                <Box sx={{ textAlign: 'center' }}>
                  <Typography variant="h3" fontWeight={800} sx={{ color: '#7C3AED' }}>{s.value}</Typography>
          ...
          <Grid container spacing={3}>
            {FEATURES.map((f) => (
              <Grid item xs={12} sm={6} md={3} key={f.title}>
                <Card sx={{ height: '100%', borderRadius: 3, border: '1px solid #ede9fe', transition: 'transform 0.2s, box-shadow 0.2s', '&:hover': { transform: 'translateY(-4px)', boxShadow: '0 12px 30px rgba(124,58,237,0.12)' } }}>
      ...
      <Box sx={{ py: 10, textAlign: 'center' }}>
        <Container maxWidth="sm">
          ...
          <Typography variant="body1" color="text.secondary" sx={{ mb: 4 }}>
            The demo is pre-loaded with sample data — contacts, deals, and activities. No setup required.
          </Typography>
          <Button variant="contained" size="large" endIcon={<ArrowForwardIcon />}
            onClick={() => navigate('/dashboard')}
      ...
      <Box sx={{ py: 3, borderTop: '1px solid #ede9fe', textAlign: 'center' }}>
